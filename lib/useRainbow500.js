...
export default function useRainbow500(reserved) {
  ...
  //
  // `retryAfterFailure` is carried into the editor so the details step can say
  // what to do about a refused card. The toast cannot: it is one line for 2.8
  // seconds, and advice somebody has to act on has to still be on screen when
  // they go to act on it.
  const resumeEditor = useCallback((saved) => {
    if (!saved) return;
    doodleDataRef.current = "";
    setEditor({ ...saved, step: "details", retryAfterFailure: true });
    setHover(null);
  }, []);
  ...
  const onNameInput = useCallback((e) => {
    ...
    setEditor((ed) => (ed ? { ...ed, name: v } : ed));
  }, []);
  const onSizeChange = useCallback((e) => {
    const v = e.target.value;
    setEditor((ed) => (ed ? { ...ed, shirtSize: v } : ed));
  }, []);
