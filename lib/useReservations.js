...
// Reservation state now lives in Supabase, not the browser: a square shows
...
// both buying the same square). This hook polls that view periodically so
// every visitor sees the same shirt, and exposes `checkout()` to kick off a
...
export default function useReservations() {
  ...
  const fetchReserved = useCallback(async () => {
    try {
      ...
      const { data, error } = await supabase.from("claimed_squares").select("*");
