...
// Ceiling on the checkout request. Generous, because the server resizes uploaded
// artwork before it replies and an overseas buyer is a long way from this box,
// but finite, because the alternative is a buyer stranded on "Redirecting to
...
// Netcash is a full top-level navigation away and back, so everything the buyer
// built (the square, the artwork, the message, their email and phone) was gone the
// moment they were redirected. A declined card meant starting over: pick a square
// again, upload the logo again, retype both fields. Card declines are ordinary,
// especially with 3DS, and rebuilding an upload from scratch is where somebody who
// is not confident with this stops trying.
//
// sessionStorage rather than localStorage on purpose. It is scoped to the tab, so
// it survives the round trip to Netcash and back but does not leave an email and
// phone number sitting on a shared machine after the tab closes.
const RESUME_KEY = "rb500.checkout.resume";
const RESUME_MAX_AGE_MS = 2 * 60 * 60 * 1000;

function saveResume(ed, ref) {
  try {
    sessionStorage.setItem(RESUME_KEY, JSON.stringify({ ed, ref, at: Date.now() }));
  } catch {
    // Private-mode Safari throws, and a quota failure is possible with large
    // artwork. Losing the ability to resume is not worth blocking a payment over.
  }
}
...
function takeResume() {
  try {
    ...
    if (!saved || !saved.ed || Date.now() - (saved.at || 0) > RESUME_MAX_AGE_MS) return null;
    return saved.ed;
  } catch {
    return null;
  }
...
export default function useReservations() {
  ...
  // The editor state recovered after a cancelled or declined payment, handed to the
  // page so it can reopen where the buyer left off.
  ...
  useEffect(() => {
    ...
    if (status === "success") {
      ...
      // Read it here and let the page decide what to do, because whether the
      // original square is still free is something only the board knows.
      setResumable(takeResume());
      fetchReserved();
    }
    if (status) {
  ...
  const checkout = useCallback(async (ed) => {
    ...
    try {
      // Bounded on purpose. This request was previously unbounded, and the
      // button reads "Redirecting to payment..." for as long as it is in
      // flight, so a request that hung left a buyer staring at that with no
      // error and no way forward. An international supporter got stuck exactly
      // there. The server also resizes artwork before replying, so the ceiling
      // has to allow for real work, not just latency.
      const ac = new AbortController();
      const timeout = setTimeout(() => ac.abort(), CHECKOUT_TIMEOUT_MS);
      ...
      // Saved before the navigation, not after: once form.submit() runs this page
      // is gone.
      saveResume(ed, data.fields && data.fields.p2);
      document.body.appendChild(form);
      form.submit();
      return true;
    } catch (e) {
      // An abort means we gave up waiting, not that the buyer did something
      // wrong, and the distinction matters: they have not been charged, because
      // nothing has reached Netcash yet. Say so, because someone who has just
      // watched a payment button stall assumes the worst.
      setCheckoutError(
        e?.name === "AbortError"
          ? "That took longer than it should have and we stopped waiting. You have not been charged. Please try again."
  ...
  return {
    ...
    redirectNotice,
    resumable,
    clearResumable: () => setResumable(null),
    paymentSuccess,
