...
export const ZONES = [
  ...
  // drift even when the exclusion shape changes.
  {
    ...
    exclude: [
      ...
      { c0: 4, c1: 5, r0: 0, r1: 0 },
      // The front yields 196 so the four sleeve squares (two per sleeve) bring
      // the campaign to 500, which means 84 excluded, so 8 cells directly under
      // the wordmark are held back. Plain fabric, sacrificed to the arithmetic.
      { c0: 0, c1: 7, r0: 14, r1: 14 },
    ],
  },
  {
    id: "front-sleeveL",
  ...
  // (35) is unchanged, so it's still exactly 350 usable squares.
  {
    ...
    // The three top corner cells on each side sat up on the shoulder slope
    // rather than the flat back. They are excluded here and re-homed as the
    // now-counted sleeve squares below, so the back still totals 350.
    exclude: [
      { c0: 0, c1: 2, r0: 0, r1: 0 }, // left shoulder corner
      { c0: 7, c1: 9, r0: 0, r1: 0 }, // right shoulder corner
    ],
  },
